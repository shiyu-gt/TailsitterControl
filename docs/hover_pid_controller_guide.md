...
1. **回收 Stage B 优化**：解决减速能力不足问题（详见 `project_overview.md` 第 6 节）
2. **悬停状态线性化**：在 `theta=90°` 配平点进行 13 状态线性化，分析悬停模态
3. **数据导出与可视化**：导出时序数据为 `.csv`/`.mat`，增加 3D 轨迹动画
4. **风扰测试**：引入持续风场或阵风模型，评估悬停抗风能力
